...
const snakeRevealAt = socialRevealAt + 650
...
function App() {
  ...
  useEffect(() => {
    if (!isSnakeVisible || isMobile) return
  ...
  }, [isSnakeVisible, isMobile, gameOverAction])

  return (
    <main className="ambient-background flex min-h-svh flex-col items-center justify-start gap-8 px-8 pt-[25svh] pb-8">
      {!isMobile && isSnakeVisible && showSnake && (
        <div className={`snake-board pointer-events-none absolute inset-5 z-0 overflow-hidden rounded-2xl md:inset-8 ${isSnakeEntering ? 'snake-enter' : ''} ${isGameOver ? 'snake-game-over' : ''}`}>
          <canvas
            ref={canvasRef}
            width={boardWidth}
            height={boardHeight}
      ...
      >
        ...
        <span className="block">
          {introduction.split('').map((character, index) => (
            <span
              key={`${character}-${index}`}
              aria-hidden="true"
              className="title-reveal-character"
              style={{ animationDelay: `${722 + index * 21}ms` }}
            >
              {character === ' ' ? '\u00A0' : character}
            </span>
          ))}
      ...
      {showSocials && (
      <nav className="entrance-reveal relative z-10 flex items-center gap-4" aria-label="Social links">
        <a
          href="https://www.linkedin.com/in/josephwangemann/"
          aria-label="LinkedIn"
          target="_blank"
          rel="noreferrer"
          className="social-icon social-icon--bloom grid size-10 place-items-center rounded-full border border-[#70b7f5] text-neutral-100 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-neutral-100"
        >
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="social-icon social-icon--bloom size-10 rounded-full focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-neutral-100"
        >
          <img
            src={sunriseAvatar}
            alt=""
      ...
      {!isMobile && isSnakeVisible && showSnake && isGameOver && (
